...
| Tampering | Double submission of a stock adjustment | the same idempotency key, stored in the adjustment transaction; a replay returns the first result and a real-thread spec sends the same request twice at once (ADR 0005, ADR 0016) | in place, slice 3 |
| Tampering | Two concurrent counts on one balance lose each other's write | pessimistic lock on the balance row; a spec checks that every ledger movement starts where the previous one ended (ADR 0004, ADR 0016) | in place, slice 3 |
