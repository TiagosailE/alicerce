...
| Rounding | Half up, at named points only: line totals, partial invoice amounts (the invoice that completes a line takes the remainder), issue costs, unit conversions, a whole-order discount allocated across its lines, and allocation. Allocation gives each part the total divided by the count rounded down and one extra cent to the first parts until the remainder is used: 10.001 in 3 is 3.334, 3.334, 3.333 (ADR 0006). |
...
| Purchasing | Purchase order, approval, partial receipts, stock entry at the order's cost, payables generated from what was received, the supplier's invoice PDF attached to the receipt, a detailed report per receipt with discounts itemized |
| Sales | Sales order with a salesperson chosen at the start of the sale, items added with a free quantity, a discount per item and one on the whole order applied on the sales screen, a free-text note, approval with reservation, partial invoicing that issues stock and creates receivables atomically, invoice cancellation, order cancellation that releases reservations, a detailed report per invoice with discounts itemized; a separate "balcao" quote (orcamento) screen prices a note with and without discount without committing anything, captured from the sales screen into a real sales order (ADR 0013) |
| Finance | Payables and receivables in installments, partial settlements with interest, fine and discount, settlement reversal, overdue status, cash and bank accounts, cash position, payment methods configured per organization (card, cash, store credit/crediario, PIX, ...) offered at the point of sale |
...
Slice 5 also carries what Tiago's 2026-09-21 research notes on the sales screen added, not yet reflected anywhere above the MVP table: a salesperson chosen at the start of the sale, distinct from whoever is operating the screen; items added with a free quantity; a discount per item and one on the whole order, both applied on the sales screen; a free-text note on the order; payment accepted through multiple organization-configured methods; and a detailed report per invoice with every discount itemized (the same detailed-report requirement applies to slice 4's receipts, above). A separate "balcao" screen builds a quote (orcamento): it prices a note with and without discount but commits nothing; the sales screen later captures a saved quote into a real sales order. ADR 0013 decides the quote is its own lightweight document, not the sales order's existing `draft` state.
