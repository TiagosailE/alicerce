...
| Purchasing: owner, admin and purchasing write orders, finance and read_only read, sales has no access; the supplier's CPF copied onto an order is stored encrypted (non-deterministic) and masked for a role that may not see partners' in full; the note is filtered from logs | in place | ADR 0017, ADR 0014, `Identity::Capabilities`, `spec/requests/api/v1/purchase_orders_spec.rb` |
| Receipts and payables: owner, admin and purchasing receive goods (Idempotency-Key, one transaction); a posted receipt and its lines are append-only (privilege revoked and a trigger for every role but the owner); a title's installments always add up to its total (a deferred constraint trigger), what a title and its installments were created with cannot be edited, and the app role cannot delete one; the payable is read by owner, admin, finance and read_only, and left out of the receipt for a role that does not read payables; the supplier's invoice number is short text (a long run of digits, such as an NF-e access key with its issuer's CNPJ, is refused), filtered from logs and recorded in the audit trail as changed without its value | in place | ADR 0017, `Identity::Capabilities`, `spec/requests/api/v1/receipts_spec.rb`, `spec/requests/api/v1/payables_spec.rb`, `spec/db/receipts_roles_spec.rb`, `spec/db/receipts_constraints_spec.rb` |
| Write limits for orders, approvals, cancellations, receipts and stock counts: 60 a minute per user and 200 in ten minutes per organization, one shared counter for all of them; receipts have their own 20 in ten minutes per organization. The organization's counters only count requests from someone allowed to write, so a role that is forbidden (the public demo's salesperson) can spend only its own allowance. Authorization runs before any lookup, so a role without access learns nothing about which ids exist | in place | `LedgerWriteLimits`, `spec/requests/api/v1/receipts_spec.rb`, `spec/requests/api/v1/purchase_orders_spec.rb` |
| Stock: sales reads balances (quantities only, cost and value are null), the ledger and its free-text notes are for every role but sales; adjustments are limited to 60 a minute per user and 200 in ten minutes per organization | in place | ADR 0016, `Identity::Capabilities`, `spec/requests/api/v1/stock_movements_spec.rb`, `spec/requests/api/v1/stock_adjustments_spec.rb` |
...
| Idempotent critical writes (stock adjustments and receipts so far): key inserted first in the command transaction, successes only, digest of method, path and body; a retry of a request that succeeded replays it even if what it depended on changed since | in place for `POST /stock_adjustments` and `POST /purchase_orders/:id/receipts`, designed for the rest | ADR 0005, ADR 0016, ADR 0017, `Idempotency`, `IdempotentWrite`, `spec/commands/inventory/adjust_stock_spec.rb`, `spec/commands/purchasing/receive_goods_spec.rb`, `spec/concurrency/purchasing/receive_goods_spec.rb` |
...
| Supplier name and document on a purchase order | `purchasing_orders.supplier_name`, `supplier_document_number` (encrypted), copied from the partner when the order is created | The order must show who it was placed with even if the partner is later corrected (ADR 0015, ADR 0017) | Kept with the order; masked per role (ADR 0014) |
| Supplier name on a receipt and a payable | `finance_titles.partner_name` (copied from the order when the payable is opened); the receipt shows the order's `supplier_name` | A payable must show who is owed even if the partner is later corrected (ADR 0015, ADR 0017); no document number is copied to either | Kept with the title; a receipt and its lines cannot be edited or deleted, so erasure of a supplier's name on them lands with the anonymization work in Milestone 2 |
| Supplier's invoice number on a receipt | `purchasing_receipts.supplier_invoice_number`, free text typed by the operator | Ties the receipt to the paper invoice for reconciliation | Kept with the receipt; filtered from logs, recorded in the audit trail as changed without its value |
