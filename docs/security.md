...
| Receipts and payables: owner, admin and purchasing receive goods (Idempotency-Key, one transaction, rate limited per user and per organization); a posted receipt and its lines are append-only (privilege revoked and a trigger for every role but the owner); the payable is read by owner, admin, finance and read_only, and left out of the receipt for a role that does not read payables; the supplier's invoice number is filtered from logs and recorded in the audit trail as changed without its value | in place | ADR 0017, `Identity::Capabilities`, `spec/requests/api/v1/receipts_spec.rb`, `spec/requests/api/v1/payables_spec.rb`, `spec/db/receipts_roles_spec.rb` |
